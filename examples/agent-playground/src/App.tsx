...
// Example behaviors (Sprint 7)
const EXAMPLE_BEHAVIORS: BehaviorDefinition[] = [
  {
    name: "coding",
    description: "Expert programming with filesystem and bash tools",
    instructions: "You are an expert programmer. Write clean, well-tested code. Use filesystem and bash tools effectively.",
    tools: [{ module: "tool-filesystem" }, { module: "tool-bash" }],
  },
  {
    name: "security-minded",
    description: "Always consider security implications",
    instructions: "Always consider security implications. Review code for vulnerabilities. Ask for approval before making changes that could affect security.",
    hooks: [{ module: "hook-approval" }],
  },
  {
    name: "research",
    description: "Web research capabilities",
    instructions: "You are a research assistant. Search the web, fetch content, and synthesize information from multiple sources.",
    tools: [{ module: "tool-web" }, { module: "tool-web-fetch" }],
  },
];

// Example recipes (Sprint 9)
const EXAMPLE_RECIPES: Record<string, { name: string; description: string; steps: RecipeStepConfig[] }> = {
  "code-review": {
    name: "code-review",
    description: "Automated code review workflow",
    steps: [
      { id: "analyze", agent: "foundation:zen-architect", prompt: "Analyze {{file_path}} for code quality, architecture, and potential issues" },
      { id: "fix", agent: "foundation:modular-builder", prompt: "Apply the recommended fixes from the analysis", requiresApproval: true },
    ],
  },
  "research": {
    ...
    steps: [
      { id: "gather", agent: "foundation:web-research", prompt: "Research {{topic}} and gather information from multiple sources" },
      { id: "summarize", agent: "foundation:technical-writer", prompt: "Summarize the research findings in a clear, concise document" },
    ],
  },
};
...
// Phase 5: Recipe step configuration
...
function App() {
  // Phase 2: Tab state
  const [activeTab, setActiveTab] = useState<'bundle' | 'behaviors' | 'recipes' | 'chat'>('bundle');
  ...
  // Phase 3: Extended bundle configuration
  const [subAgents, setSubAgents] = useState<SubAgentConfig[]>([]);
  const [hooks, setHooks] = useState<ModuleConfig[]>([]);
  const [mcpServers, setMcpServers] = useState<McpServerConfig[]>([]);

  // Phase 4: Behaviors state
  const [behaviors, setBehaviors] = useState<{
    defined: BehaviorDefinition[];
    selected: string[];
  }>({
    defined: [],
    selected: [],
  ...
  // Phase 5: Recipe state
  const [recipe, setRecipe] = useState<{
    name: string;
    description: string;
    steps: RecipeStepConfig[];
    context: Record<string, string>;
    execution: RecipeExecution | null;
  ...
  // ============================================================================
  // PHASE 1: Register convenience API handlers
  // ============================================================================
  
  // Register onThinking handler
  useEffect(() => {
    if (!showThinking) return;

    const handler = (thinking: { isThinking: boolean; content: string }) => {
    ...
    return () => client.offThinking(handler);
  }, [showThinking]);

  // Register onApproval handler
  useEffect(() => {
    ...
    return () => {
      // No offApproval method, just set to null
    };
  }, []);
  ...
  const applyConfig = async () => {
    ...
    try {
      ...
      // Phase 3: Add sub-agents to bundle
      if (subAgents.length > 0) {
        bundle.agents = subAgents.map((sa) => ({
          name: sa.name,
      ...
      // Phase 3: Add hooks to bundle
      if (hooks.length > 0) {
        bundle.hooks = hooks;
      }

      // Phase 3: Add MCP servers to bundle
      if (mcpServers.length > 0) {
        bundle.mcpServers = mcpServers;
      }

      // Create new session with runtime bundle and behaviors
      const session = await client.createSession({ 
        bundle,
        behaviors: behaviors.selected,
      });
      ...
      console.log(`[Session] Applied behaviors:`, behaviors.selected);
  ...
  const handleEvent = (event: Event, messageId: string) => {
    switch (event.type) {
      ...
      // PHASE 1: Removed manual thinking.delta handler - now using onThinking()
      ...
      // PHASE 1: Removed manual approval.required handler - now using onApproval()
      ...
      default:
        if (![" content.end", "content.start", "ack", "result", "thinking.delta", "approval.required"].includes(event.type)) {
          console.log("Event:", event.type, event.data);
        }
    }
  };

  // PHASE 1: Updated approval handler to use resolve function
  ...
  const updateSubAgent = (index: number, field: keyof SubAgentConfig, value: string | string[]) => {
    setSubAgents((prev) =>
  ...
  const addHook = () => {
    const module = prompt("Enter hook module name (e.g., 'hook-logging'):");
    if (module && module.trim()) {
      setHooks((prev) => [...prev, { module: module.trim() }]);
    }
  };
  ...
  const addMcpServer = () => {
    setMcpServers((prev) => [
      ...prev,
      { type: "stdio", command: "", args: [], env: {} } as McpServerStdio,
    ]);
  };
  ...
  const updateMcpType = (index: number, type: "stdio" | "http" | "sse") => {
    setMcpServers((prev) =>
      prev.map((server, i) => {
        if (i !== index) return server;
        if (type === "stdio") {
          return { type: "stdio", command: "", args: [], env: {} } as McpServerStdio;
        } else if (type === "http") {
          return { type: "http", url: "", headers: {} } as McpServerHttp;
        } else {
          return { type: "sse", url: "", headers: {} } as McpServerSse;
        }
      })
    );
  };
  ...
  // ============================================================================
  // PHASE 4: Behaviors management
  // ============================================================================
  
  const loadExampleBehavior = (behavior: BehaviorDefinition) => {
    setBehaviors((prev) => ({
      ...prev,
      defined: [...prev.defined.filter((b) => b.name !== behavior.name), behavior],
    }));
  };

  const toggleBehavior = (name: string, selected: boolean) => {
    setBehaviors((prev) => ({
      ...prev,
      selected: selected
        ? [...prev.selected, name]
  ...
  const createCustomBehavior = () => {
    const name = prompt("Behavior name:");
    if (!name || !name.trim()) return;

    const instructions = prompt("Behavior instructions:");
  ...
  const removeBehavior = (name: string) => {
    setBehaviors((prev) => ({
      defined: prev.defined.filter((b) => b.name !== name),
  ...
  const updateRecipeStep = (index: number, field: keyof RecipeStepConfig, value: string | boolean | number) => {
    setRecipe((prev) => ({
      ...prev,
  ...
  const buildRecipe = (): RecipeDefinition | null => {
    if (!recipe.name || recipe.steps.length === 0) return null;

    const builder = new RecipeBuilder(recipe.name)
      .description(recipe.description || "No description")
      .version("1.0.0");

    recipe.steps.forEach((step) => {
      builder.step(step.id, (s) => {
        s.agent(step.agent).prompt(step.prompt);
        if (step.requiresApproval) {
          s.requiresApproval(`Approve ${step.id}?`);
        }
        if (step.timeout) {
  ...
  const executeRecipe = async () => {
    const recipeDef = buildRecipe();
    if (!recipeDef) {
      alert("Please provide a recipe name and at least one step");
      return;
    }

    try {
      setRecipe((prev) => ({ ...prev, progress: [], execution: null }));

      const execution = await client.executeRecipe(recipeDef.name, recipe.context);
      ...
      execution.on("step.started", (event) => {
        setRecipe((prev) => ({
          ...
          progress: [
            ...prev.progress,
            { stepId: event.step_id, status: "running" },
          ],
        }));
      });
      ...
      execution.on("step.completed", (event) => {
        setRecipe((prev) => ({
          ...
          progress: prev.progress.map((p) =>
            p.stepId === event.step_id
              ? { ...p, status: "completed", output: JSON.stringify(event.output) }
              : p
          ),
        }));
      });
      ...
      setRecipe((prev) => ({ ...prev, execution }));
    } catch (err) {
      console.error("Failed to execute recipe:", err);
      alert(`Failed to execute recipe: ${(err as Error).message}`);
    }
  };

  const loadExampleRecipe = (recipeName: string) => {
    const example = EXAMPLE_RECIPES[recipeName];
    if (example) {
      setRecipe((prev) => ({
        ...prev,
        name: example.name,
  ...
  const updateRecipeContext = (key: string, value: string) => {
    setRecipe((prev) => ({
      ...prev,
  ...
  const removeRecipeContext = (key: string) => {
    setRecipe((prev) => {
  ...
  const renderBundleBuilderTab = () => (
    <aside className="config-panel">
      ...
      <div className="config-section">
        <h3>Sub-Agents</h3>
        <p className="section-description">Configure spawnable agents within the bundle</p>
        {subAgents.map((agent, idx) => (
          <div key={idx} className="sub-agent-item">
            ...
            <textarea
              ...
              onChange={(e) => updateSubAgent(idx, "instructions", e.target.value)}
              rows={2}
            />
            <button className="btn-icon" onClick={() => removeSubAgent(idx)}>🗑️</button>
          </div>
        ))}
        <button className="btn-secondary" onClick={addSubAgent} style={{ width: "100%" }}>
          + Add Sub-Agent
        </button>
      </div>
      ...
      <div className="config-section">
        <h3>Hooks</h3>
        <p className="section-description">Add hook modules for event handling</p>
        {hooks.map((hook, idx) => (
          <div key={idx} className="hook-item">
            <span>🪝 {hook.module}</span>
            <button className="btn-icon" onClick={() => removeHook(idx)}>×</button>
          </div>
        ))}
        <button className="btn-secondary" onClick={addHook} style={{ width: "100%" }}>
          + Add Hook
        </button>
      </div>
      ...
      <div className="config-section">
        <h3>MCP Servers</h3>
        <p className="section-description">Connect to Model Context Protocol servers</p>
        {mcpServers.map((server, idx) => (
          <div key={idx} className="mcp-server-item">
            <select
              className="config-input"
              value={server.type}
              onChange={(e) => updateMcpType(idx, e.target.value as "stdio" | "http" | "sse")}
            >
            ...
            {server.type === "stdio" && (
              <input
                className="config-input"
                placeholder="Command"
                value={(server as McpServerStdio).command}
            ...
            {(server.type === "http" || server.type === "sse") && (
              <input
                className="config-input"
                placeholder="URL"
                value={(server as McpServerHttp | McpServerSse).url}
                onChange={(e) => updateMcpServer(idx, "url", e.target.value)}
              />
            )}
            <button className="btn-icon" onClick={() => removeMcpServer(idx)}>×</button>
          </div>
        ))}
        <button className="btn-secondary" onClick={addMcpServer} style={{ width: "100%" }}>
          + Add MCP Server
        </button>
      </div>
    ...
    <aside className="config-panel">
      <div className="config-section">
        ...
        <div className="behavior-examples">
          ...
          {EXAMPLE_BEHAVIORS.map((behavior) => (
            <button
              key={behavior.name}
              className="preset-btn"
              onClick={() => loadExampleBehavior(behavior)}
            >
              {behavior.name}
            </button>
          ))}
        ...
        <div className="behavior-list">
          <h4>Defined Behaviors</h4>
          {behaviors.defined.length === 0 ? (
            <p className="empty-message">No behaviors defined. Load an example or create a custom one.</p>
          ) : (
            behaviors.defined.map((behavior) => (
              <div key={behavior.name} className="behavior-item">
                <label className="behavior-checkbox">
                  <input
                    type="checkbox"
                    checked={behaviors.selected.includes(behavior.name)}
                    onChange={(e) => toggleBehavior(behavior.name, e.target.checked)}
                  />
                  <div>
                    <strong>{behavior.name}</strong>
                    {behavior.description && <p className="behavior-description">{behavior.description}</p>}
                  </div>
                </label>
                <button className="btn-icon" onClick={() => removeBehavior(behavior.name)}>×</button>
              </div>
            ))
          )}
        </div>

        <button className="btn btn-secondary" onClick={createCustomBehavior} style={{ width: "100%" }}>
          + Create Custom Behavior
        </button>

        {behaviors.selected.length > 0 && (
          <div className="selected-behaviors">
            <h4>Selected: {behaviors.selected.length}</h4>
            <p className="section-description">
              These behaviors will be applied when you create/update the agent.
            </p>
          </div>
        )}
      </div>
  ...
  const renderRecipesTab = () => (
    <aside className="config-panel">
      <div className="config-section">
        ...
        <p className="section-description">
          Multi-step workflow orchestration with approval gates.
        </p>
          ...
          {recipe.steps.map((step, idx) => (
            <div key={idx} className="recipe-step">
              <input
                ...
                placeholder="Step ID"
                value={step.id}
                onChange={(e) => updateRecipeStep(idx, "id", e.target.value)}
              />
              <input
                className="config-input"
                placeholder="Agent (e.g., foundation:zen-architect)"
                value={step.agent}
                onChange={(e) => updateRecipeStep(idx, "agent", e.target.value)}
              />
              <textarea
                className="config-input"
                placeholder="Prompt (use {{variable}} for context)"
                value={step.prompt}
                onChange={(e) => updateRecipeStep(idx, "prompt", e.target.value)}
                rows={2}
              />
              <label className="recipe-step-option">
                <input
                  type="checkbox"
                  checked={step.requiresApproval}
                  onChange={(e) => updateRecipeStep(idx, "requiresApproval", e.target.checked)}
                />
                Requires Approval
              </label>
              <button className="btn-icon" onClick={() => removeRecipeStep(idx)}>×</button>
            </div>
          ))}
          <button className="btn-secondary" onClick={addRecipeStep} style={{ width: "100%" }}>
            + Add Step
          </button>

          <h4>Context Variables</h4>
          <div className="recipe-context">
            {Object.entries(recipe.context).map(([key, value]) => (
              <div key={key} className="context-item">
                ...
                <input
                  className="config-input"
                  value={value}
                  onChange={(e) => updateRecipeContext(key, e.target.value)}
                  style={{ width: "50%" }}
                />
                <button className="btn-icon" onClick={() => removeRecipeContext(key)}>×</button>
              </div>
            ))}
            <button
        ...
        <div className="recipe-actions">
          <button className="btn btn-primary" onClick={executeRecipe} disabled={!recipe.name || recipe.steps.length === 0}>
            ▶️ Execute Recipe
          </button>
        </div>

        {recipe.progress.length > 0 && (
          <div className="recipe-progress">
            <h4>Execution Progress</h4>
            {recipe.progress.map((p) => (
              <div key={p.stepId} className={`progress-item status-${p.status}`}>
                <strong>{p.stepId}:</strong> {p.status}
                {p.output && <pre className="progress-output">{p.output}</pre>}
                {p.error && <pre className="progress-error">{p.error}</pre>}
              </div>
            ))}
          </div>
        )}
        ...
        <div className="recipe-examples">
          ...
          {Object.keys(EXAMPLE_RECIPES).map((name) => (
            <button key={name} className="preset-btn" onClick={() => loadExampleRecipe(name)}>
              {name}
            </button>
          ))}
        </div>
    ...
    <div className="app">
      ...
      <header className="header">
        ...
        <div className="tabs">
          ...
          <button
            className={`tab-btn ${activeTab === 'behaviors' ? 'active' : ''}`}
      ...
      {/* Phase 2: Conditional Tab Rendering */}
      {activeTab === 'bundle' && renderBundleBuilderTab()}
      {activeTab === 'behaviors' && renderBehaviorsTab()}
      {activeTab === 'recipes' && renderRecipesTab()}

      {/* Chat Panel - always visible when chat tab is active */}
      {activeTab === 'chat' && (
        <main className="chat-panel">
          {sessionId ? (
                ...
                <div className="agent-info">
                  ...
                  <div>
                    ...
                    <div className="agent-status">
                      ...
                      {behaviors.selected.length > 0 && ` • ${behaviors.selected.length} behaviors`}
                    </div>
                  </div>
                </div>
            ...
            <div className="empty-state">
              ...
              <p>
                Go to Bundle Builder, set up your agent's configuration, then click
      ...
      {/* Show chat panel alongside other tabs when session exists */}
